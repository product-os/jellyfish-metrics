...
const { metrics } = require('@balena/node-metrics-gatherer');
...
exports.startServer = (context, port) => {
	...
	const b64enc = Buffer.from(`monitor:${environment.metrics.token}`).toString(
		'base64',
	);
	...
	server.on('listening', () => {
		logger.info(
			context,
			`Metrics server listening on port ${server.address().port}`,
		);
	});
	server.on('error', (err) => {
...
exports.markQueueConcurrency = () => {
	metrics.gauge(
		descriptions.names.WORKER_CONCURRENCY,
		environment.queue.concurrency,
	);
};
...
exports.measureMirror = async (integration, fn) => {
	...
	const result = await measureAsync(
		descriptions.names.MIRROR_DURATION,
		labels,
		fn,
	).catch((err) => {
...
exports.measureTranslate = async (integration, fn) => {
	...
	const results = await measureAsync(
		descriptions.names.TRANSLATE_DURATION,
		labels,
		fn,
	);
	return results;
	...
	metrics.inc(descriptions.names.STREAMS_LINK_QUERY_TOTAL, 1, {
		...
		type:
			_.has(change, ['type']) && _.isString(change.type)
				...
				: 'unknown',
		card:
			_.has(change, ['after', 'type']) && _.isString(change.after.type)
