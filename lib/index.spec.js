...
ava.before(async (test) => {
	...
	metrics.measureTranslate(test.context.integration, test.context.func);
	metrics.measureHttpQuery(test.context.func);
	metrics.measureHttpType(test.context.func);
	metrics.measureHttpId(test.context.func);
	metrics.measureHttpSlug(test.context.func);
	metrics.measureHttpAction(test.context.func);
	metrics.measureHttpWhoami(test.context.func);
});
...
ava(
	'.markCardInsert() should increment the card insert counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_card_insert_total{type="${test.context.cardType}"} 1`,
			),
		);
		...
		test.truthy(
			result.body.includes(
				`jf_card_insert_total{type="${test.context.cardType}"} 2`,
			),
		);
	},
);
...
ava(
	'.markCardUpsert() should increment the card upsert counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_card_upsert_total{type="${test.context.cardType}"} 1`,
			),
		);
		...
		test.truthy(
			result.body.includes(
				`jf_card_upsert_total{type="${test.context.cardType}"} 2`,
			),
		);
	},
);
...
ava(
	'.markCardReadFromDatabase() should increment the card read from database counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_card_read_total{type="${test.context.cardType}",source="database"} 1`,
			),
		);
		...
		test.truthy(
			result.body.includes(
				`jf_card_read_total{type="${test.context.cardType}",source="database"} 2`,
			),
		);
	},
);
...
ava(
	'.markCardReadFromCache() should increment the card read from cache counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_card_read_total{type="${test.context.cardType}",source="cache"} 1`,
			),
		);
		...
		test.truthy(
			result.body.includes(
				`jf_card_read_total{type="${test.context.cardType}",source="cache"} 2`,
			),
		);
	},
);
...
ava('.markBackSync() should increment the back sync counter', async (test) => {
	...
	test.truthy(
		result.body.includes(
			`jf_back_sync_total{type="${test.context.integration}"} 1`,
		),
	);
	...
	test.truthy(
		result.body.includes(
			`jf_back_sync_total{type="${test.context.integration}"} 2`,
		),
	);
});

ava(
	'.markActionRequest() should increment the action request counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_worker_action_request_total{type="${test.context.action}"} 1`,
			),
		);
		...
		test.truthy(
			result.body.includes(
				`jf_worker_action_request_total{type="${test.context.action}"} 2`,
			),
		);
	},
);
...
ava(
	'.markJobAdd() and .markJobDone() should increment and decrement the worker saturation gauge',
	async (test) => {
		metrics.markJobAdd(test.context.action, test.context.context.id);
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_worker_saturation{type="${test.context.action}",worker="${test.context.context.id}"} 1`,
			),
		);
		...
		test.truthy(
			result.body.includes(
				`jf_worker_saturation{type="${test.context.action}",worker="${test.context.context.id}"} 0`,
			),
		);
	},
);
...
ava(
	'.markSqlGenTime() should update SQL query generation time metrics',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
...
ava(
	'.markQueryTime() should update SQL query execution time metrics',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
...
ava(
	'.markStreamOpened() and .markStreamClosed() should increment and decrement the stream saturation gauge',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_streams_saturation{actor="${test.context.actor}",table="${test.context.table}"} 1`,
			),
		);
		...
		test.truthy(
			result.body.includes(
				`jf_streams_saturation{actor="${test.context.actor}",table="${test.context.table}"} 0`,
			),
		);
	},
);
...
ava(
	'.markStreamLinkQuery() should increment the stream link query counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_streams_link_query_total{table="${test.context.table}",actor="${test.context.actor}",` +
...
ava(
	'.markStreamError() should increment the stream error counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_streams_error_total{actor="${test.context.actor}",table="${test.context.table}"} 1`,
			),
		);
		...
		test.truthy(
			result.body.includes(
				`jf_streams_error_total{actor="${test.context.actor}",table="${test.context.table}"} 2`,
			),
		);
	},
);
...
ava(
	'.markQueueConcurrency() should expose the queue concurrency setting',
	async (test) => {
		const result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_worker_concurrency ${environment.queue.concurrency}`,
			),
		);
	},
);
...
ava(
	'.measureMirror() should update the mirror execution counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_mirror_duration_seconds_count{type="${test.context.integration}"} 1`,
			),
		);
		...
		test.truthy(
			result.body.includes(
				`jf_mirror_duration_seconds_count{type="${test.context.integration}"} 2`,
			),
		);
	},
);
...
ava(
	'.measureTranslate() should update translate execution counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(
			result.body.includes(
				`jf_translate_duration_seconds_count{type="${test.context.integration}"} 1`,
			),
		);
		...
		test.truthy(
			result.body.includes(
				`jf_translate_duration_seconds_count{type="${test.context.integration}"} 2`,
			),
		);
	},
);
...
ava(
	'.measureHttpQuery() should increment the /query request counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(result.body.includes('jf_http_api_query_total 1'));
		test.truthy(
		...
		test.truthy(
			result.body.includes('jf_http_api_query_duration_seconds_count 2'),
		);
	},
);

ava(
	'.measureHttpType() should increment the /type request counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(result.body.includes('jf_http_api_type_total 1'));
		test.truthy(
		...
		test.truthy(
			result.body.includes('jf_http_api_type_duration_seconds_count 2'),
		);
	},
);

ava(
	'.measureHttpId() should increment the /id request counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(result.body.includes('jf_http_api_id_total 1'));
		test.truthy(
		...
		test.truthy(
			result.body.includes('jf_http_api_id_duration_seconds_count 2'),
		);
	},
);

ava(
	'.measureHttpSlug() should increment the /slug request counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(result.body.includes('jf_http_api_slug_total 1'));
		test.truthy(
		...
		test.truthy(
			result.body.includes('jf_http_api_slug_duration_seconds_count 2'),
		);
	},
);

ava(
	'.measureHttpAction() should increment the /action request counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(result.body.includes('jf_http_api_action_total 1'));
		test.truthy(
		...
		test.truthy(
			result.body.includes('jf_http_api_action_duration_seconds_count 2'),
		);
	},
);

ava(
	'.measureHttpWhoami() should increment the /whoami request counter',
	async (test) => {
		let result = await getMetrics();
		test.truthy(result.body.includes('jf_http_whoami_query_total 1'));
		test.truthy(
		...
		test.truthy(
			result.body.includes('jf_http_whoami_query_duration_seconds_count 2'),
		);
	},
);
